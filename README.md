...
- \_\_dataset\_\_.py
    ...
    Before using the code provided, note that the image size must be changed for the input LR and output SR/HR to the desired shape on the __dataset.py__ file after the imports:
...
- __training.py__ and __testing.py__
    ...
    In each training.py and testing.py files we must define the path of the OCR to be used (download the pre-trained OCRs [here](https://github.com/Valfride/lpr-rsr-ext/releases/download/OCR_pre-trained_models/saved_models.zip)\):
